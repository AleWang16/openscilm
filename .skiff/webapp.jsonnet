...
function(
    ...
    // Run through some tests to make sure everything is okay.
    assert util.assertTests();
    assert util.assertLoginConfig(config);
    ...
    // Here we define the ingresses used for the .apps.allenai.org domain.
    //
    // appsIngress may be used with Skiff Login, while appsIngressNoLogin is used
    // for routes that are explicitly configured to not support Skiff Login.
    local appsTLS = util.getTLSConfig(fullyQualifiedName, hosts);
    local appsIngress = {
        ...
        metadata: {
            ...
            labels: labels,
            annotations: annotations + appsTLS.ingressAnnotations + util.getAuthAnnotations(config, '.apps.allenai.org') + {
                'nginx.ingress.kubernetes.io/ssl-redirect': 'true'
            }
        },
        spec: {
            tls: [ appsTLS.spec + { hosts: hosts } ],
            rules: [
                {
                    host: host,
                    http: {
                        paths: util.ingressPathsSupportingLogin(config, fullyQualifiedName, proxyPort),
                    }
                } for host in hosts
            ]
        }
    };
    local appsIngressNoLogin = {
        apiVersion: 'networking.k8s.io/v1',
        kind: 'Ingress',
        metadata: {
            name: fullyQualifiedName + '-nologin',
            namespace: namespaceName,
            labels: labels,
            annotations: annotations + appsTLS.ingressAnnotations + {
                'nginx.ingress.kubernetes.io/ssl-redirect': 'true'
            }
        },
        spec: {
            ...
            rules: [
                {
                    host: host,
                    http: {
                        paths: util.ingressPathsNoLogin(config, fullyQualifiedName, proxyPort),
                    }
                } for host in hosts
            ]
        }
    ...
    // Here we define the ingresses used for the .allen.ai domain.
    //
    // allenAIIngress may be used with Skiff Login, while allenAINoLoginIngress
    // is used for routes that are explicitly configured to not support Skiff Login.
    ...
    local allenAIIngressNoLogin = {
        apiVersion: 'networking.k8s.io/v1',
        kind: 'Ingress',
        metadata: {
            name: fullyQualifiedName + '-allen-dot-ai-nologin',
            namespace: namespaceName,
            labels: labels,
            annotations: annotations + allenAITLS.ingressAnnotations + {
                'nginx.ingress.kubernetes.io/ssl-redirect': 'true'
            }
        },
        spec: {
    ...
    // Here we define the ingresses used for the .semanticscholar.org domain.
    //
    // scholarIngress may be used with Skiff Login, while scholarNoLoginIngress
    // is used for routes that are explicitly configured to not support Skiff Login.
    ...
    local scholarNoLoginIngress = {
        apiVersion: 'networking.k8s.io/v1',
        kind: 'Ingress',
        metadata: {
            name: fullyQualifiedName + '-scholar-nologin',
            namespace: namespaceName,
            labels: labels,
            annotations: annotations + scholarTLS.ingressAnnotations + {
                'nginx.ingress.kubernetes.io/ssl-redirect': 'true'
            }
        },
        spec: {
    ...
    // Initial objects that we unconditionally include in the spec.
    ...
    // Intermediate result that potentially incorporates scholarIngress.
    local result1 =
      if std.length(scholarHosts) > 0 then
          initialObjects + [ scholarIngress ]
      else
          initialObjects;

    // Intermediate result that potentially incorporates NoLogin ingresses.
    local result2 =
      if 'nologin_path_prefixes' in config then
          if std.length(scholarHosts) > 0 then
              result1 + [ appsIngressNoLogin, allenAIIngressNoLogin, scholarNoLoginIngress ]
          else
              result1 + [ appsIngressNoLogin, allenAIIngressNoLogin ]
      else
          result1;

    // This is the final result that holds all the resources we intend
    // to have in the spec.
    result2
